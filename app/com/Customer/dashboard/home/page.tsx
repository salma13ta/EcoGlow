    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12">
        <div className="bg-[#F0EDE6] rounded-[2rem] overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-8 items-center p-6 sm:p-12 relative">
          ...
          >
            <img src={heroData.image} alt="EcoGlow Premium Hero" className="w-full h-full object-cover object-center" />
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <motion.div variants={fadeInContainer} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {categories.map((cat) => (
            <Link href="/com/Customer/dashboard/shop" key={cat.id} className="contents">
              <motion.div variants={fadeInUp} whileHover={{ y: -6 }} className="group cursor-pointer relative rounded-2xl overflow-hidden aspect-[4/5] bg-white shadow-sm border border-[#E4EBE3]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <motion.div variants={fadeInContainer} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <motion.div key={prod.id} variants={fadeInUp} className="bg-white rounded-2xl p-3 sm:p-4 border border-[#E4EBE3] shadow-sm flex flex-col justify-between group relative">
              <div>
                <div className="relative aspect-square w-full rounded-xl overflow-hidden bg-[#FAF8F5] mb-4">
                  ...
                  <motion.button whileTap={{ scale: 0.85 }} className="absolute top-2 right-2 p-1.5 rounded-full bg-white/80 backdrop-blur-sm border border-neutral-100 text-[#5C6E5B] hover:text-red-500 transition-colors">
                    <Heart size={15} />
                  </motion.button>
                </div>
                <h3 className="text-xs sm:text-sm font-bold text-[#1E3E1A] line-clamp-1 mb-1">{prod.name}</h3>
                <div className="flex items-center gap-1 mb-3">
                  <div className="flex text-amber-400">{[...Array(prod.rating)].map((_, i) => <Star key={i} size={12} fill="currentColor" />)}</div>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-[#F0F4EF]">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-[#1E3E1A]/5 hover:bg-[#1E3E1A] text-[#1E3E1A] hover:text-white transition-colors duration-300 text-xs font-semibold px-3 py-1.5 rounded-full flex items-center gap-1">
                  <Plus size={14} /> Add
                </motion.button>
              </div>
            </motion.div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 mb-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-[#6B8E73] text-white rounded-[2rem] p-6 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-md">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left z-10 max-w-xl">
            <img src={quizBannerData.image} alt="Quiz Icon" className="w-16 h-16 rounded-2xl object-cover shrink-0 shadow-sm border border-white/20" />
            <div>
